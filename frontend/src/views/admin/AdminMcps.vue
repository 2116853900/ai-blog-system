...
import type { Mcp, Page } from '../../api/types'

const loading = ref(false)
const editing = ref<Partial<Mcp> | null>(null)
const page = ref(0)
const size = 20
const pageData = ref<Page<Mcp> | null>(null)

const items = computed(() => pageData.value?.content ?? [])

async function load() {
  loading.value = true
  try { pageData.value = await adminApi.mcps({ page: page.value, size }) } finally { loading.value = false }
}
...
<template>
  <div>
    ...
    <p v-else-if="!items.length" class="muted">没有 MCP。</p>
